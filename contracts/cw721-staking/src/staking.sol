...
contract StakingPoolFour is Ownable {
    ...
    mapping (uint => uint) public daysRates;
    mapping (address => uint) public depositedDays;
   ...
   */
    ...
    function daysRateSet(uint daysInTimestamp, uint _rewardRate) public onlyDAOorAdmin returns(bool){
     daysRates[daysInTimestamp] = _rewardRate;
     return true;
    ...
    function getUnclaimedDivs(address _holder) public view returns (uint) {
        ...
        uint numberOfDays = depositedDays[_holder];
        uint numberOfDaysRate = daysRates[numberOfDays];
        
        uint unclaimedDivs = stakedAmount
                            .mul(numberOfDaysRate)
                            .mul(timeDiff)
                            .div(rewardInterval)
                            .div(1e4);
    ...
    function stake(uint amountToStake, uint numberOfDays) public {
        
        require(daysRates[numberOfDays] > 0, "Invalid number of days");
        ...
        depositedDays[msg.sender] = numberOfDays;
    ...
    function unstake(uint amountToWithdraw) public {
        ...
        require((block.timestamp).sub(stakingTime[msg.sender]) >= depositedDays[msg.sender], "Your request is rather too early");
